...
async function readCodexUsageOrRpc(deps = {}) {
  ...
  const rpcReader = deps.readOnlyCredentials ? async () => { throw errorWithStatus('unauthorized', 'Sign in again using Codex'); } : (deps.readCodexRpc || readCodexRpc);
...
async function fetchLiveCodexAccount(deps = {}, nowMs = Date.now(), managedAccounts = []) {
  ...
  const payload = deps.readOnlyCredentials ? result.payload : await withCodexOAuthResetCredits(result.payload, deps, result.oauthAuthSnapshot);
  const authIdentity = result.oauthAuthSnapshot
    ? codexAuthIdentity(result.oauthAuthSnapshot.auth)
