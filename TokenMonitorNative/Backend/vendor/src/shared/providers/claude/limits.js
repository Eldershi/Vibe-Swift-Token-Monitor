...
const CLAUDE_OAUTH_CLIENT_ID = ''; // Native distribution omits unused upstream OAuth client constants.
...
async function readClaudeCredentials(deps = {}) {
  ...
  if ((deps.platform || process.platform) === 'darwin' && deps.readMacKeychain !== false) {
    const text = await readMacKeychainSecret('Claude Code-credentials', deps).catch(error => {
...
function readMacKeychainSecret(service, deps = {}) {
  if (deps.readMacKeychainSecret) return deps.readMacKeychainSecret(service);
  // Read-only collectors must never fall back to an interactive security CLI.
  if (deps.readOnlyCredentials) return Promise.resolve('');
...
async function refreshClaudeAccessToken(refreshToken, deps = {}) {
  if (!CLAUDE_OAUTH_CLIENT_ID) throw errorWithStatus('unauthorized', 'Sign in again using Claude Code');
...
async function refreshClaudeCredentials(currentCredentials, deps = {}) {
  if (deps.readOnlyCredentials) throw errorWithStatus('unauthorized', 'Sign in again using Claude Code');
  const platform = deps.platform || process.platform;
...
async function fetchClaudeLimits(options = {}, deps = {}) {
  deps = { ...deps, readOnlyCredentials: options.readOnlyCredentials === true || deps.readOnlyCredentials === true };
  const nowMs = (deps.now || Date.now)();
  const platform = deps.platform || process.platform;
  ...
  try {
    ...
    if (error?.code === 'CLAUDE_IDENTITY_UNAVAILABLE') throw error;
    if (deps.readOnlyCredentials || !shouldTryClaudeCliFallback(error)) throw error;
    try {
