...
设置中的“数据”页显示服务、最近成功采集时间和额度来源状态。关闭窗口或退出界面后继续采集，下次登录由系统启动后台。系统如要求允许后台运行，使用设置中的入口进入“登录项与扩展”。
...
安装脚本仅替换 beta；识别旧固定名称或唯一的顶层版本化安装，将上一应用包保存到 `~/Applications/Token Monitor 历史版本/`，文件名包含版本、构建号和时间。多个顶层版本化 Beta 同时存在时拒绝安装，避免误替换。稳定版 0.4.1 不参与替换。后台状态与历史保持兼容；若未来升级修改数据格式，应先备份 beta 数据目录。

卸载前先按下方步骤停用 Hub 同步，再在 beta 设置中点击“停用后台”，然后退出界面，将 beta 应用移到废纸篓即可。数据默认保留，稳定版可以继续使用。需要回退应用包时，同样先停用同步和后台并退出，再将历史版本文件夹中选定的旧应用包移回 `~/Applications/`，恢复为 `Token Monitor Native Beta.app` 或其版本化文件名（顶层只保留一个 Beta）；随后启用后台，核对该版本的同步能力。
...
`verify-beta-service.py` 自动识别唯一的顶层安装，或用 `--app` 指定应用包。
...
设置的“数据”页中 Hub 同步分组支持验证连接、绑定已有本机设备、启停同步和查看“仅本机／共享 Hub”。本机采集与界面退出相互独立；Hub 离线继续采集，界面保留上次汇总和真实同步时间。
...
1. 在 beta“数据”设置的 Hub 同步分组停用同步，确认已停用；需要回退整个后台时再停用独立后台。
